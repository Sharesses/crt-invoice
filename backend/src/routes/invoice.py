...
from src.models.invoice import Invoice, InvoiceLine, Product, Supplier, PriceHistory
...
class InvoiceLine(db.Model):
    ...
    ocr_confidence = db.Column(db.Float, nullable=True)  # Score de confiance pour cette ligne
    validation_status = db.Column(db.String(50), default='pending')  # pending, validated, rejected
    validated_by = db.Column(db.String(100), nullable=True)  # Utilisateur qui a validé
    validated_at = db.Column(db.DateTime, nullable=True)
    
    # Correspondance produit
    product_match_confidence = db.Column(db.Float, nullable=True)  # Confiance de la correspondance produit
    suggested_products = db.Column(db.Text, nullable=True)  # JSON des produits suggérés
